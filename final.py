...
def create_pyecharts_pie_chartmod(country_specific_data,lab):
    # Labels for pie chart
    labels = lab
    
    # Extract data values
    sizes = country_specific_data[labels].values[0]
    
    # Create pie chart using pyecharts
    pie = Pie()
    
    # Add data to the pie chart and increase the radius
    pie.add(
        "",
        [list(z) for z in zip(labels, sizes)],
        radius=["30%", "60%"]  # Increased radius for a larger pie chart
    )
    ...
    pie.set_global_opts(
        ...
        legend_opts=opts.LegendOpts(
            is_show=True, 
            textstyle_opts=opts.TextStyleOpts(color='white',font_size=16  )  # Change legend text color
        )
    )
    
    # Set series options with minimal customization
    pie.set_series_opts(
        label_opts=opts.LabelOpts(font_size=14, color='white'),  # Set font size and color for the labels
        itemstyle_opts=opts.ItemStyleOpts(border_radius=12, border_color="#fff", border_width=1)
    )
    
    return pie

def create_pyecharts_pie_chart(labels, values):
    ...
    pie.add(
        "COVID-19 Stats",
        [list(z) for z in zip(labels, values)],  # Zipping labels and values
        radius="50%"  # Setting radius to 50% for a standard pie chart
    )
    ...
    pie.set_global_opts(
        title_opts=opts.TitleOpts(
            ...
            pos_left="center"  # Correct argument for positioning
        ),
        tooltip_opts=opts.TooltipOpts(trigger="item"),
        legend_opts=opts.LegendOpts(
            orient="vertical", 
            pos_left="left" , # Correct argument for positioning the legend
            textstyle_opts=opts.TextStyleOpts(color='white', font_size=16)
        )
    )
    
    # Set series options with emphasis effects
    pie.set_series_opts(
       label_opts=opts.LabelOpts(
            formatter="{b}: {c} ({d}%)",  # Show labels as "name: value (percentage)"
            color='white',  # Change label text color
            font_size=14  # Optionally adjust font size
        ),  # Show labels as "name: value (percentage)"
        emphasis_opts=opts.EmphasisOpts(
            itemstyle_opts=opts.ItemStyleOpts(  # Set emphasis options
                border_color="#fff",  # Optional: border color
                border_width=1,  # Optional: border width
            ),
            # Shadow effects in emphasis settings
...
def global_section():
    ...
    if selected == "Continent and Country Selection":
        ...
        if continent_sidebar != 'None':
            ...
            df2 = df1['Country/Region'].to_list()
            print(df2[0])
            # Show country selection in the sidebar
            country_sidebar = st.sidebar.multiselect('Select countries:', sorted(df2), default=df2[0])
        ...
        if country_sidebar:
            ...
            for country in country_sidebar:
                ...
                # st.pyplot(fig)
                pie_chart = create_pyecharts_pie_chartmod(country_specific_data,labels)
                # Display the chart with specific width and height to avoid overflow
                st_pyecharts(pie_chart, height="600px", width="600px")
    ...
    # Section 2: COVID-19 Statistics Over Time
        ...
        if selected_view == 'Pie Chart':
            # Assuming 'filtered_df' contains the required COVID-19 data
            latest_date = filtered_df['Date'].max()
            latest_data = filtered_df[filtered_df['Date'] == latest_date][['Confirmed', 'Deaths', 'Recovered', 'Active']].iloc[0]
            
            # Prepare labels and values
            labels = ['Confirmed', 'Deaths', 'Recovered', 'Active']
            values = latest_data.values.tolist()  # Ensure values are in list format
            
            # Create the Pyecharts pie chart
            pie_chart = create_pyecharts_pie_chart(labels, values)
            
            # Display the Pyecharts pie chart in Streamlit
            st_pyecharts(pie_chart, height="600px", width="600px")
